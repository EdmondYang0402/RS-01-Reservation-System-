...
const { t, tm } = useI18n();
onMounted(async () => {
  rooms.value = await getAvailability({
...
<template>
  <section class="hero">
    ...
    <div class="container hero-content">
      ...
      <h1 class="pre-line">{{ t("home.heroTitle") }}</h1>
  ...
  <section class="section home-rooms">
    <div class="container">
      <SectionHeading
        ...
        :description="t('home.roomsDescription')"
      />
      <div class="room-grid">
        <RoomCard v-for="room in rooms" :key="room.id" :room="room" />
      </div>
      <div class="section-action">
        <RouterLink class="text-link" to="/rooms">{{
          t("home.viewAllRooms")
        }}</RouterLink>
      </div>
    </div>
  ...
  <section class="promise-section">
    <div class="container promise-grid">
      <div v-for="(item, index) in tm('home.promises')" :key="item.title">
        <span>0{{ index + 1 }}</span>
        <h3>{{ item.title }}</h3>
