...
const load = async () => {
  loading.value = true;
  try {
    rows.value = await getTodayDepartures();
  } finally {
    loading.value = false;
  }
};
  ...
  <section class="management-page">
    <div class="container">
      ...
      <el-table v-loading="loading" :data="rows"
        ...
        ><el-table-column :label="t('frontDesk.assignedRoom')"
          ><template #default="scope">{{
            scope.row.assignedRoomNumber || t("common.unassigned")
          }}</template></el-table-column
        ...
        ><el-table-column :label="t('common.actions')"
          ><template #default="scope"
            ><el-button link type="primary" @click="openCheckOut(scope.row)">{{
