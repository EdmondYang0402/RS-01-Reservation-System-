...
watch(
  ...
  async (open) => {
    ...
    try {
      rooms.value = await getAvailableRooms(props.reservation.roomTypeId);
    } finally {
...
<template>
  <el-dialog
    ...
    ><p class="dialog-note">
      {{ t("reservation.reservationNo") }}：{{ reservation?.reservationNo }}
    </p>
    <el-form label-position="top"
      ><el-form-item :label="t('frontDesk.availableRooms')"
        ><el-select
          v-model="form.roomId"
          v-loading="roomLoading"
          :placeholder="t('frontDesk.noAvailableRoom')"
          ><el-option
            v-for="room in rooms"
            :key="room.id"
            :label="room.roomNumber"
            :value="room.id" /></el-select></el-form-item></el-form
    ><template #footer
      ...
      ><el-button
        ...
        :loading="loading"
        @click="submit"
        >{{ t("frontDesk.submitCheckIn") }}</el-button
      ></template
    ></el-dialog
  >
</template>
